...
export class RegistryService {
  ...
  getImageTags(repository: string): Observable<{ repository: string; tags: string[] }> {
  ...
  addTag(repository: string, sourceTag: string, newTag: string): Observable<{ message: string }> {
  ...
  getEmptyRepositories(): Observable<{ empty_repositories: string[]; count: number }> {
    return this.http.get<{ empty_repositories: string[]; count: number }>(`${this.BASE}/empty-repositories`);
  }

  purgeEmptyRepositories(): Observable<{ message: string; purged: string[]; errors: any[] }> {
    return this.http.delete<{ message: string; purged: string[]; errors: any[] }>(`${this.BASE}/empty-repositories`);
  }
}
