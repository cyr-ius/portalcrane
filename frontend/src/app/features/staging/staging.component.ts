...
import { StagingService, StagingJob, DockerHubResult } from '../../core/services/staging.service';
...
@Component({
    ...
    <div class="p-4">
      <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
          ...
          <p class="text-muted small mb-0">Pull from Docker Hub → Scan → Push to Registry</p>
        </div>
      </div>
        ...
        <div class="col-lg-5">
          ...
          <div class="card border-0 mb-3">
            <div class="card-header border-0">
              <h6 class="fw-semibold mb-0"><i class="bi bi-search me-2"></i>Search Docker Hub</h6>
            </div>
            <div class="card-body">
              <div class="input-group mb-2">
                ...
                />
                <button class="btn btn-primary" (click)="searchDockerHub()" [disabled]="searching()">
                        ...
                        <div class="flex-grow-1">
                          <div class="d-flex align-items-center gap-1 mb-1">
                            ...
                            @if (result.is_official) {
                              <span class="badge bg-primary-subtle text-primary" style="font-size:0.6rem">OFFICIAL</span>
                            }
                          </div>
                          <div class="text-muted" style="font-size:0.72rem">{{ result.description | slice:0:80 }}{{ result.description.length > 80 ? '...' : '' }}</div>
                          <div class="d-flex gap-2 mt-1">
                            <span class="text-muted" style="font-size:0.65rem">
                              <i class="bi bi-star-fill text-warning"></i> {{ formatCount(result.star_count) }}
                            </span>
                            <span class="text-muted" style="font-size:0.65rem">
                              <i class="bi bi-download"></i> {{ formatCount(result.pull_count) }}
                            </span>
                          </div>
                        </div>
                      </div>
          ...
          <div class="card border-0">
            <div class="card-header border-0">
              <h6 class="fw-semibold mb-0"><i class="bi bi-cloud-arrow-down me-2"></i>Pull Image</h6>
            </div>
            <div class="card-body">
              <div class="mb-3">
        ...
        <div class="col-lg-7">
          <div class="card border-0">
            <div class="card-header border-0 d-flex align-items-center justify-content-between">
              <h6 class="fw-semibold mb-0"><i class="bi bi-list-task me-2"></i>Pipeline Jobs</h6>
              <button class="btn btn-sm btn-outline-secondary" (click)="loadJobs()">
                  ...
                  @for (job of jobs(); track job.job_id) {
                    <div class="job-card p-3 rounded">
                      ...
                      <div class="d-flex align-items-start justify-content-between mb-2">
                        <div>
                          <div class="fw-semibold small">{{ job.image }}:{{ job.tag }}</div>
                          <div class="text-muted" style="font-size:0.7rem">{{ job.job_id }}</div>
                        </div>
                      ...
                      <div class="mb-2">
                        <div class="progress mb-1" style="height:6px">
                          <div
                            class="progress-bar"
                            [class.bg-success]="job.status === 'done' || job.status === 'scan_clean'"
                            [class.bg-danger]="job.status === 'failed' || job.status === 'scan_infected'"
                            [class.progress-bar-striped]="['pulling','scanning','pushing'].includes(job.status)"
                            [class.progress-bar-animated]="['pulling','scanning','pushing'].includes(job.status)"
                      ...
                      <!-- Scan result -->
                      @if (job.scan_result) {
                        <div class="small mb-2 p-2 rounded bg-body-secondary font-monospace text-break" style="font-size:0.7rem">
                          {{ job.scan_result }}
                        </div>
                      }
  ...
  styles: [`
    ...
    .search-results-list { max-height: 280px; overflow-y: auto; }
    .search-result-item {
      cursor: pointer;
      transition: background 0.1s;
      border: 1px solid var(--pc-border);
    }
    .search-result-item:hover { background: var(--pc-nav-hover); }
    .search-result-item.selected { background: var(--pc-nav-active-bg); border-color: var(--pc-accent); }
    .job-card {
      background: var(--pc-main-bg);
      border: 1px solid var(--pc-border);
    }
    .job-list { max-height: 600px; overflow-y: auto; }
  `],
})
export class StagingComponent implements OnInit, OnDestroy {
  ...
  startPull() {
    ...
    this.staging.pullImage(this.pullImage(), this.pullTag() || 'latest').subscribe({
      next: (job) => {
