...
Portalcrane uses URL-encoded slashes (`%2F`) in API paths to represent image names
containing namespaces (e.g. `biocontainers/swarm` → `/api/registry/images/biocontainers%2Fswarm/tags`).
Most reverse proxies normalize URIs by default and decode `%2F` into `/`, which breaks these routes.
The following snippets show how to disable this normalization.

### Traefik v2 / v3

Add `allowEncodedSlashes: true` to the router label:

```yaml
# docker-compose.yml
labels:
  - "traefik.enable=true"
  - "traefik.http.routers.portalcrane.rule=Host(`your.domain.com`)"
  - "traefik.http.routers.portalcrane.entrypoints=websecure"
  - "traefik.http.routers.portalcrane.tls.certresolver=letsencrypt"
  - "traefik.http.routers.portalcrane.allowEncodedSlashes=true" # required
  - "traefik.http.services.portalcrane.loadbalancer.server.port=8080"
```

Or in the static configuration file `traefik.yml`:
...
```
frontend portalcrane_front
    bind *:443 ssl crt /etc/ssl/certs/cert.pem
    # Disable URI normalization to preserve %2F encoded slashes
    option accept-invalid-http-request
    default_backend portalcrane_back

backend portalcrane_back
    server portalcrane 127.0.0.1:8080 check
```
...
server {
    ...
    location / {
        # Disable automatic decoding of encoded slashes
        merge_slashes off;
...
> **Note:** `merge_slashes off` prevents Nginx from collapsing `//` but does **not**
> prevent decoding of `%2F`. For Nginx, the recommended approach is to use
> `proxy_pass` with a variable: `set $upstream http://127.0.0.1:8080; proxy_pass $upstream$request_uri;`
> which bypasses URI normalization entirely.

### Caddy

```
your.domain.com {
    # encode directive with no options preserves %2F
    reverse_proxy 127.0.0.1:8080
}
```

Caddy preserves encoded slashes natively — no special configuration needed.
